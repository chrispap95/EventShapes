...
FinalParticles = GenParticles[
    (GenParticles_Status == 1)
    & (GenParticles.pt > 1)
    & (np.abs(GenParticles.eta) < 2.5)
]
FinalParticles_ParentId = GenParticles_ParentId[
    (GenParticles_Status == 1)
    & (GenParticles.pt > 1)
    & (np.abs(GenParticles.eta) < 2.5)
]
...
ax.hist(
    FromScalarParticles.energy.max() / IsrParticles.energy.mean(),
    bins=100,
    histtype="step",
    color="b",
)
