...
h_suep = np.zeros(20)
h_isr = np.zeros(20)
eff_suep = np.zeros(20)
eff_isr = np.zeros(20)
h_suep_bst = np.zeros(20)
h_isr_bst = np.zeros(20)
eff_suep_bst = np.zeros(20)
eff_isr_bst = np.zeros(20)
...
for ievt in range(GenParticles_Status.size):
    ...
    E_fromScalar_bst = np.sort(genJetsAK2_fromScalar_bst.energy)
    E_ISR_bst = np.sort(genJetsAK2_ISR_bst.energy)
