...
for ievt in range(GenParticles_Status.size):
    ...
    hist_dR_pt.fill(suepsUtilities.deltar(scalarParticle.eta, scalarParticle.phi, jetsAK15[0].eta, jetsAK15[0].phi))
    hist_dR_mult.fill(suepsUtilities.deltar(scalarParticle.eta, scalarParticle.phi, suepJet.eta, suepJet.phi))

eff = 100*k/n
sigma_eff = 100*((k+1)*(k+2)/((n+2)*(n+3))-(k+1)**2/(n+2)**2)
print(120*"=")
...
ax.plot(hist_dR_pt.axes[0].edges[:-1], hist_dR_pt.view(), drawstyle='steps-post', color='black', linestyle='-', label='Highest $p_{T}$ jet');
ax.plot(hist_dR_mult.axes[0].edges[:-1], hist_dR_mult.view(), drawstyle='steps-post', color='r', linestyle='-', label='Highest multiplicity jet');
