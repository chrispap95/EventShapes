...
for i in range(1,bins+1):
    ...
    nEntr_bkg[i-1][1] = hist_sph_bkg_leadPt_ak4_suep[::sum]-hist_sph_bkg_leadPt_ak4_suep[:i:sum]
    nEntr_bkg[i-1][2] = hist_sph_bkg_leadPt_ak4_isr[::sum]-hist_sph_bkg_leadPt_ak4_isr[:i:sum]

# Add bkg to distribution plot
ax2.hist(sph_bkg_leadPt, bins=bins, range=(min, max), weights=CrossSection_bkg, histtype='step',
         color='black', linestyle='-', linewidth=2, label='QCD')
ax2.hist(sph_bkg_leadPt_ak4_suep, bins=bins, range=(min, max), weights=CrossSection_bkg, histtype='step',
         color='black', linestyle=':', linewidth=2)
ax2.hist(sph_bkg_leadPt_ak4_isr, bins=bins, range=(min, max), weights=CrossSection_bkg, histtype='step',
         color='black', linestyle='-.', linewidth=2)
...
for i in range(1,bins+1):
    ...
    nEntr_sig[i-1][1] = hist_sph_sig_leadPt_ak4_suep[::sum]-hist_sph_sig_leadPt_ak4_suep[:i:sum]
    nEntr_sig[i-1][2] = hist_sph_sig_leadPt_ak4_isr[::sum]-hist_sph_sig_leadPt_ak4_isr[:i:sum]
...
ax2.hist(sph_sig_leadPt, bins=bins, range=(min, max), weights=CrossSection_sig,
         histtype='step', color=colorVal, label='$mMed = %s\,$GeV, $temp = %s\,$K'%(mMed,temp),
         linestyle='-', linewidth=2)
ax2.hist(sph_sig_leadPt_ak4_suep, bins=bins, range=(min, max), weights=CrossSection_sig,
         histtype='step', color=colorVal,
         linestyle=':', linewidth=2)
ax2.hist(sph_sig_leadPt_ak4_isr, bins=bins, range=(min, max), weights=CrossSection_sig,
         histtype='step', color=colorVal,
         linestyle='-.', linewidth=2)
