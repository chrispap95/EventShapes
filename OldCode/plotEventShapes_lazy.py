...
datasets = {
    base1 + 'Autumn18.QCD_HT1000to1500_TuneCP5_13TeV-madgraphMLM-pythia8_RA2AnalysisTree.root': 'TreeMaker2/PreSelection',
    base1 + 'Autumn18.QCD_HT1500to2000_TuneCP5_13TeV-madgraphMLM-pythia8_RA2AnalysisTree.root': 'TreeMaker2/PreSelection',
...
for ievt in [x for x in range(len(events['Tracks.fCoordinates.fX'])) if x > 101065 or x < 100000]:
    if ievt%1000 == 0:
...
efficiency = np.concatenate((np.ones(100000),np.ones(100000)*98934/100000,np.ones(100000)))
...
ax.hist(evtShape, bins=25, range=(0, 1), weights=CrossSection*efficiency, histtype='step', label='minus 0', color=colorVal)
colorVal = scalarMap.to_rgba(values[1])
ax.hist(evtShape1, bins=25, range=(0, 1), weights=CrossSection*efficiency, histtype='step', label='minus 5', color=colorVal)
colorVal = scalarMap.to_rgba(values[2])
ax.hist(evtShape2, bins=25, range=(0, 1), weights=CrossSection*efficiency, histtype='step', label='minus 10', color=colorVal)
colorVal = scalarMap.to_rgba(values[3])
ax.hist(evtShape3, bins=25, range=(0, 1), weights=CrossSection*efficiency, histtype='step', label='minus 20', color=colorVal)
colorVal = scalarMap.to_rgba(values[4])
ax.hist(evtShape4, bins=25, range=(0, 1), weights=CrossSection*efficiency, histtype='step', label='minus 40', color=colorVal)
colorVal = scalarMap.to_rgba(values[5])
ax.hist(evtShape5, bins=25, range=(0, 1), weights=CrossSection*efficiency, histtype='step', label='minus 80', color=colorVal)
