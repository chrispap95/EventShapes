...
for i in range(1, sphericityBins + 1):
    ...
    for j in range(sph_bkg_dPhi.shape[1]):
        nEntr_sig[i - 1, j + 1] = (
            hist_sph_sig_dPhi[j][::sum] - hist_sph_sig_dPhi[j][:i:sum]
        )
        nEntr_bkg[i - 1, j + 1] = (
            hist_sph_bkg_dPhi[j][::sum] - hist_sph_bkg_dPhi[j][:i:sum]
        )
    for j in range(sph_bkg_relE.shape[1]):
        nEntr_sig[i - 1, j + sph_bkg_relE.shape[1] + 1] = (
            hist_sph_sig_relE[j][::sum] - hist_sph_sig_relE[j][:i:sum]
        )
        nEntr_bkg[i - 1, j + sph_bkg_relE.shape[1] + 1] = (
            hist_sph_bkg_relE[j][::sum] - hist_sph_bkg_relE[j][:i:sum]
        )
    nEntr_sig[i - 1, numberOfCases_relE_dPhi + 1] = (
        hist_sph_sig_leadPt[::sum] - hist_sph_sig_leadPt[:i:sum]
    )
    nEntr_bkg[i - 1, numberOfCases_relE_dPhi + 1] = (
        hist_sph_bkg_leadPt[::sum] - hist_sph_bkg_leadPt[:i:sum]
    )
    nEntr_sig[i - 1, numberOfCases_relE_dPhi + 2] = (
        hist_sph_sig_highMult[::sum] - hist_sph_sig_highMult[:i:sum]
    )
    nEntr_bkg[i - 1, numberOfCases_relE_dPhi + 2] = (
        hist_sph_bkg_highMult[::sum] - hist_sph_bkg_highMult[:i:sum]
    )
    nEntr_sig[i - 1, numberOfCases_relE_dPhi + 3] = (
        hist_sph_sig_noLowMult[::sum] - hist_sph_sig_noLowMult[:i:sum]
    )
    nEntr_bkg[i - 1, numberOfCases_relE_dPhi + 3] = (
        hist_sph_bkg_noLowMult[::sum] - hist_sph_bkg_noLowMult[:i:sum]
    )
...
ax1.plot(
    sphericitySpace,
    significance[significanceMode, :, i],
    ".-",
    label="$|\Delta\phi|>%.2f$" % (dPhiValues[i - 1]),
    color=colorVal,
)
i = 12
...
ax1.plot(
    sphericitySpace,
    significance[significanceMode, :, numberOfCases_relE_dPhi + 1],
    ".-",
    label="lead Pt jet",
    color=colorVal,
)
...
ax1.plot(
    sphericitySpace,
    significance[significanceMode, :, numberOfCases_relE_dPhi + 2],
    ".-",
    label="high mult. jet",
    color=colorVal,
)
...
ax2.hist(
    ...
    bins=25,
    range=(0, 1),
    weights=xs_signal[mMed] * np.ones(sph_sig_dPhi[:, i].size),
    histtype="step",
    color=colorVal,
    linestyle=":",
    linewidth=2,
...
colorVal = scalarMap.to_rgba(values[1])
ax2.hist(
    sph_bkg_relE[:, i - sph_bkg_dPhi.shape[1] - 1],
    bins=25,
    range=(0, 1),
    weights=CrossSection_bkg,
    histtype="step",
...
ax2.hist(
    sph_sig_relE[:, i - sph_bkg_dPhi.shape[1] - 1],
    bins=25,
    range=(0, 1),
    weights=xs_signal[mMed]
    * np.ones(sph_sig_relE[:, i - sph_bkg_dPhi.shape[1] - 1].size),
    histtype="step",
    color=colorVal,
    linestyle=":",
    linewidth=2,
...
ax2.hist(
    ...
    bins=25,
    range=(0, 1),
    weights=xs_signal[mMed] * np.ones(sph_sig_leadPt.size),
    histtype="step",
    color=colorVal,
    linestyle=":",
    linewidth=2,
...
ax2.hist(
    ...
    bins=25,
    range=(0, 1),
    weights=xs_signal[mMed] * np.ones(sph_sig_highMult.size),
    histtype="step",
    color=colorVal,
    linestyle=":",
    linewidth=2,
...
ax1.text(
    center,
    top,
    "signal is mMed%s_%s" % (mMed, decay),
