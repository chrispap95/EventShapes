...
def signalRun(mMed, hist1, hist2):
    ...
    base = '/Users/chrispap/'
    datasets = [
        base + 'PrivateSamples.SUEP_2018_mMed-%d_mDark-2_temp-2_decay-darkPhoHad_13TeV-pythia8_n-100_0_RA2AnalysisTree.root'%(mMed),
    ]
...
hist_400_suep = bh.Histogram(bh.axis.Regular(100, 1e-5, 1, transform=bh.axis.transform.log))
hist_400_isr = bh.Histogram(bh.axis.Regular(100, 1e-5, 1, transform=bh.axis.transform.log))
hist_750_suep = bh.Histogram(bh.axis.Regular(100, 1e-5, 1, transform=bh.axis.transform.log))
hist_750_isr = bh.Histogram(bh.axis.Regular(100, 1e-5, 1, transform=bh.axis.transform.log))
hist_1000_suep = bh.Histogram(bh.axis.Regular(100, 1e-5, 1, transform=bh.axis.transform.log))
hist_1000_isr = bh.Histogram(bh.axis.Regular(100, 1e-5, 1, transform=bh.axis.transform.log))
...
ax.plot(hist_400_suep.axes[0].edges[:-1], hist_400_suep.view()/hist_400_suep.sum(), drawstyle='steps', color='b', linestyle='-', label='mMed 400 GeV');
ax.plot(hist_400_isr.axes[0].edges[:-1], hist_400_isr.view()/hist_400_suep.sum(), drawstyle='steps', color='b', linestyle='--');
ax.plot(hist_750_suep.axes[0].edges[:-1], hist_750_suep.view()/hist_750_suep.sum(), drawstyle='steps', color='r', linestyle='-', label='mMed 750 GeV');
ax.plot(hist_750_isr.axes[0].edges[:-1], hist_750_isr.view()/hist_750_suep.sum(), drawstyle='steps', color='r', linestyle='--');
ax.plot(hist_1000_suep.axes[0].edges[:-1], hist_1000_suep.view()/hist_1000_suep.sum(), drawstyle='steps', color='g', linestyle='-', label='mMed 1000 GeV');
ax.plot(hist_1000_isr.axes[0].edges[:-1], hist_1000_isr.view()/hist_1000_suep.sum(), drawstyle='steps', color='g', linestyle='--');
