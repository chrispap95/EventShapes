...
def makeJets(tracks, R, p=-1, mode="old"):
    ...
    for track in tracks:
        vectors[i] = np.array(
            (track.pt, track.eta, track.phi, track.mass),
            np.dtype([("pT", "f8"), ("eta", "f8"), ("phi", "f8"), ("mass", "f8")]),
        )
        i += 1
...
def isrTagger(jets, warn=False, warnThresh=130, multiplicity="high"):
    if len(jets) == 0:
        ...
        return
    if len(jets) == 1:
        return uproot_methods.TLorentzVectorArray.from_ptetaphie(
            [jets[0].pt], [jets[0].eta], [jets[0].phi], [jets[0].e]
        )
    mult0 = len(jets[0])
    mult1 = len(jets[1])
    ...
    if mult0 < mult1:
        ...
        if multiplicity == "high":
            ...
            return
    return uproot_methods.TLorentzVectorArray.from_ptetaphie(
        [jets[index].pt], [jets[index].eta], [jets[index].phi], [jets[index].e]
    )
