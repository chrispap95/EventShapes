...
class TreeMakerProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        output["nTracksHist"].fill(
            dataset="CMSSW GEN", nTracks=nTracksGEN, weight=weights
        )
        output["nTracksHist"].fill(
            dataset="CMSSW RECO", nTracks=nTracksRECO, weight=weights
        )
...
if __name__ == "__main__":
    ...
    hist.plot1d(
        tmOut["nTracksHist"]["CMSSW RECO"], ax=ax, clear=False, error_opts=data_err_opts
    )
