...
for i in range(1,sphericityBins+1):
    ...
    eff_sig[i-1,numberOfCases_relE_dPhi+3] = hist_sph_sig_noLowMult[:i:sum]/hist_sph_sig_noLowMult[::sum]
    eff_bkg[i-1,numberOfCases_relE_dPhi+3] = hist_sph_bkg_noLowMult[:i:sum]/hist_sph_bkg_noLowMult[::sum]
...
ax.plot(1.0-eff_sig[:,0], 1.0-eff_bkg[:,0], '.-', label='all tracks', color=colorVal)
...
colorVal = scalarMap.to_rgba(values[i])
ax.plot(1.0-eff_sig[:,i], 1.0-eff_bkg[:,i], '.-', label='$E_{i}/\sum_{j}E_{j}<%.4f$'%(relEValues[i-sph_bkg_dPhi.shape[1]-1]), color=colorVal)
colorVal = scalarMap.to_rgba(values[numberOfCases_relE_dPhi+1])
ax.plot(1.0-eff_sig[:,numberOfCases_relE_dPhi+1], 1.0-eff_bkg[:,numberOfCases_relE_dPhi+1], '.-', label='lead Pt jet', color=colorVal)
colorVal = scalarMap.to_rgba(values[numberOfCases_relE_dPhi+2])
ax.plot(1.0-eff_sig[:,numberOfCases_relE_dPhi+2], 1.0-eff_bkg[:,numberOfCases_relE_dPhi+2], '.-', label='high mult. jet', color=colorVal)
colorVal = scalarMap.to_rgba(values[numberOfCases_relE_dPhi+3])
ax.plot(1.0-eff_sig[:,numberOfCases_relE_dPhi+3], 1.0-eff_bkg[:,numberOfCases_relE_dPhi+3], '.-', label='remove ISR jet', color=colorVal)
